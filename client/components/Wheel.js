...
export default function Wheel (props) {
    ...
    const colorArray = array.map((el,i)=>{
        ...
        if (array.length<15){
            return <div style={styling} key={i} className={classBox}><p id="remove-skew">{array[i]}</p></div>
        } else {
    ...
    return (
        <>
            ...
            <div className={`wheel ${props.animationClass}`} onAnimationEnd={()=>{
                // snaps wheel back
                setTimeout(()=>{
                ...
                props.handleChooseParticpant(reversed[Math.floor((props.randomNum/360)*array.length)])
                props.setWheelChose(reversed[Math.floor((props.randomNum/360)*array.length)])
                }}>
                {colorArray}
            </div>
        </>
